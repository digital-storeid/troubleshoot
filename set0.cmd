...
::  To disable changing edition if current edition doesn't support Troubleshoot, change the value to 1 from 0 or run the script with "/HWID-NoEditionChange" parameter
...
%eline%
echo Troubleshoot is not supported for Windows Server.
...
if not [%%#]==[] (echo "%%#" | find "127.69" %nul1% && (echo "%%#" | find "127.69.%ver%" %nul1% || set old=1))
)

if defined old (
echo ________________________________________________
%eline%
echo Not for Sale -- DIGITAL.STOREID --
echo ________________________________________________
echo:
...
call :dk_color %Blue% "[%altedition%] Edition product key will be used to enable Troubleshoot."
echo:
)
...
call :dk_color %Blue% "At the time of writing this, Troubleshoot was not supported for this device."
...
::  Below code is used to get alternate edition name and key if current edition doesn't support Troubleshoot
